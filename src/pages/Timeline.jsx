  ...
  return (
    <SimplePageLayout>
      ...
      <motion.section
        ...
        className="mb-12 grid grid-cols-2 gap-3 rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-700 to-cyan-800 p-6 text-white shadow-sm dark:border-cyan-400/25 dark:from-slate-900 dark:via-blue-950 dark:to-cyan-950 sm:grid-cols-4 md:gap-4 md:p-8"
      >
        <p className="col-span-full mb-2 text-center text-xl font-bold md:text-2xl">
          {t('timeline.countdown')}
        </p>
        <div className="rounded-xl bg-white/15 px-3 py-4 text-center ring-1 ring-white/20 backdrop-blur">
      ...
      <div className="relative space-y-8 before:absolute before:left-4 before:top-2 before:h-[calc(100%-2rem)] before:w-1 before:rounded-full before:bg-gradient-to-b before:from-orange-400 before:to-emerald-500 md:before:left-6">
        {timelineEvents.map((ev, i) => (
          <motion.article
            ...
            className="relative ms-12 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:ms-16 md:p-8"
          >
            ...
            <p className="text-lg font-bold text-blue-800 dark:text-cyan-300 md:text-xl">{ev.date}</p>
            <h2 className="mt-2 text-2xl font-extrabold text-slate-900 dark:text-white md:text-3xl">
              {ev.title[lang]}
            </h2>
            <p className="mt-2 text-lg text-slate-600 dark:text-slate-300 md:text-xl">{ev.desc[lang]}</p>
          </motion.article>
        ))}
      </div>
    </SimplePageLayout>
